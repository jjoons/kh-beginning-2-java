...
public class StringTokenMain {
  public static void main(String[] args) {
    // StringTokenizer 클래스
    //   문자열 구분자를 기준으로 토큰(token)이라는
    //   여러 개의 문자열로 잘라내는데 사용
    //   구분자를 넘겨주지 않을 경우 공백으로 설정
    ...
    // 반복자
    //   split("") 메소드같은 경우 구분자가 하나만 넣을 수 있음
    ...
    try {
      String str2 = "Welc%ome/to/The%Java*HelloWorld";
...
/*
 * 먼저 split 메소드는 String클래스에 속해있는 메소드이고,
 * StringTokenizer는 java.util에 포함되어 있는 클래스이다.
 * 
 * 구분자를 split는 정규 표현식으로 구분하고, StringTokenizer는 문자로 받는다.
 * 
 * split는 결과 값이 문자열 배열이지만, stringtokenizer는 객체이다.
 * 
 * split는 빈문자열을 토큰으로 인식하는 반면, StringTokenizer는
 * 빈 문자열을 토큰으로 인식하지 않는다. (뒤에 코드 예제)
 * 
 * 성능은 split 보다 StringTokenizer 가 좋다.
 * 
 * split은 데이터를 토큰으로 잘라낸 결과를 배열에 담아서 반환하기 때문에
